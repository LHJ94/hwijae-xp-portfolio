...
- 대표 사례 6개를 문제·목표 → 역할 → 실행 과정 → 구축한 결과 순서로 정리
- 기존 공개 경험 서사를 사례를 열 때 한 번만 로드. 실패하면 기본 프로젝트의 기여·산출물을 표시
- 내부 검증 메모와 큰 빈 이미지 칸 대신 접을 수 있는 자료 안내 제공. 미검증 수치는 노출하지 않음
- 5개 프로그램에서 실제 사례로 이동, 사례 간 다음 탐색과 직접 링크 진입 시 홈 복귀 제공
...
- 대표 사례 구조: `app.js`의 `renderProject()`. 문제·실행·결과 서사는 `data/work-archive-public.json`의 해당 `project_id` 항목을 사용
...
2026-09-22: 사례 6개·프로그램 연결 5곳, 서사 데이터 1회 지연 로드, 직접 링크와 다음 사례, 390/320px 화면, 키보드 펼치기, 데이터 실패 시 기본 표시와 로딩 중 화면 전환 검증. 정상 흐름의 JS·파일 오류 0건. Node 문법·기존 데이터 검사와 `git diff --check` 통과.
